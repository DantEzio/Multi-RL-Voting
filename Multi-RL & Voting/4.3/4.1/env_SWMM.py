...
class env_SWMM:
    def __init__(self, date_time, date_t, QVI):
    ...
    def reset(self,raindata,rainid,train):
        #每一次batch都新生成一个新的降雨
        ...
        if train:
            if self.QVI:
                #对比HC，生成HC算法在降雨时间内的策略和flooding数据，flooding数据作为每步模拟的基准数据
                if 'f'+str(rainid)+'.xlsx' in os.listdir('./sim/QVI/'):
                    self.total_in_compare=pd.read_excel('./sim/QVI/in'+str(rainid)+'.xlsx').values[:,1]
                    self.flooding_compare=pd.read_excel('./sim/QVI/f'+str(rainid)+'.xlsx').values[:,1]
                    #print(self.total_in_compare)
                else:  
                    change_rain.copy_result('./sim/compare_tem_HC.inp',self.orf_rain+'.inp')
                    self.flooding_compare,self.total_in_compare=[],[]
                    for i in range(1,len(self.date_time)):
                        set_datetime.set_date(self.sdate,self.edate,self.stime,self.date_time[i],'./sim/compare_tem_HC.inp')
                        #reward2使用的标准比对baseline
                        self.simulation('./sim/compare_tem_HC.inp')
                        t1,f1,_,_,_,_=get_rpt.get_rpt('./sim/compare_tem_HC.rpt')
                        self.total_in_compare.append(t1)
                        self.flooding_compare.append(f1)
                    
                    pd.DataFrame(self.total_in_compare).to_excel('./sim/QVI/in'+str(rainid)+'.xlsx')
                    pd.DataFrame(self.flooding_compare).to_excel('./sim/QVI/f'+str(rainid)+'.xlsx')
        else:
            self.total_in_compare=raindata
            self.flooding_compare=raindata
        ...
        rain_sum=sum(raindata[self.date_t[self.iten]:self.date_t[self.iten+1]])
        
        h=0
        for pool in self.pool_list:
            h+=self.pool_d[pool]
        h/=len(self.pool_list)
        state=np.array([outflow/(0.001+total_in),flooding/(0.001+total_in),store/(0.001+total_in),h,rain_sum])
    ...
    def step(self,a,raindata):
        ...
        if total_in==0:
            ...
            reward_sum+=(total_in-flooding)/(total_in)#HC算法的flooding数据作为DDQN每步模拟的基准数据
